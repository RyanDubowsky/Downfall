...
namespace Downfall.Code.RestSiteOptions;
...
public class GemRestSiteOption(Player owner) : CustomRestSiteOption(owner)
{
    ...
    public override async Task<bool> OnSelect()
    {
        ...
        var gemHolder = PileType.Deck.GetPile(Owner).Cards.Where(c => c is GuardianCardModel { FreeSlots: > 0 }).ToList();
        ...
        if (CardSelectCmd.ShouldSelectLocalCard(Owner))
        {
            ...
            RunManager.Instance.PlayerChoiceSynchronizer.SyncLocalChoice(Owner, choiceId, PlayerChoiceResult.FromMutableDeckCards(cardModel));
        }
        else
            cardModel = (await RunManager.Instance.PlayerChoiceSynchronizer.WaitForRemoteChoice(Owner, choiceId)).AsDeckCards().ToList();
