...
/// <summary>
///     Extensions to <see cref="RewardSynchronizer" /> to provide public getters to internal properties and common reward
///     functions
/// </summary>
...
public static class RewardSynchronizerExtensions
{
    ...
    internal static List<BufferedCustomRewardMessage> BufferedCustomRewardMessages(
        this RewardSynchronizer rewardSynchronizer)
    {
        return _bufferedCustomRewardMessages;
    }

    /// <summary>
    ///     Add a <see cref="CustomRewardMessage" /> to the combat buffer
    /// </summary>
    public static void BufferCustomRewardMessage(this RewardSynchronizer rewardSynchronizer,
        CustomRewardMessage message, ulong senderId)
    {
    ...
    {
        return rewardSynchronizer._playerCollection.GetPlayer(rewardSynchronizer._localPlayerId);
    }
    ...
    {
        return rewardSynchronizer._playerCollection;
    }
    ...
    {
        return rewardSynchronizer._messageBuffer;
    }
    ...
    /// </summary>
    public static async Task<bool> DoCardTransform(this RewardSynchronizer rewardSynchronizer, Player player,
        int amount = 1, bool upgrade = false)
    {
        var prefs = new CardSelectorPrefs(new LocString("gameplay_ui", "COMBAT_REWARD_CARD_TRANSFORM"), amount)
        {
            Cancelable = true,
        ...
        BaseLibMain.Logger.Debug(
            $"Current combat state for transform rewards is: IsEnding={CombatManager.Instance.IsEnding}");
        foreach (var card in cards)
        {
            var newCard = CardFactory.CreateRandomCardForTransform(card, false, player.RunState.Rng.Niche);
            ...
            BaseLibMain.Logger.Debug($"Player {player.NetId} transformed {card.Id} in their deck into {newCard.Id}" +
                                     (upgrade ? " and upgraded it." : "."));
        }
    ...
    {
        foreach (var bufferedMessage in __instance.BufferedCustomRewardMessages())
            __instance.MessageBuffer()?.CallHandlersOfType(bufferedMessage.message.GetType(), bufferedMessage.message,
